...
void main() {
  group('Download Test', () {
    test('=> good url', () async {
      ...
      var result = (await http.get(uri)).bodyBytes;

      expect(await loadFromRemote(url, null, 5, const Duration(milliseconds: 100), 1.0, const Duration(seconds: 5), null, null, printError: true), result);

      url = 'https://github.com/dart-lang/site-shared/raw/master/src/_assets/image/flutter/logo/default.svg';
      result = (await http.get(uri)).bodyBytes;
    ...
    test('=> good url with progress', () async {
      ...
      result = (await http.get(realUri)).bodyBytes;
    ...
    test('=> bad url with skip 404 retry', () async {
      ...
      expect(await loadFromRemote(url, null, 5, const Duration(milliseconds: 100), 1.0, const Duration(seconds: 5), null, null, skipRetryStatusCode: [404], printError: true), null);
    });
    test('=> not a url', () async {
      var url = '/GitHub-Marks.png';

      expect(await loadFromRemote(url, null, 0, const Duration(milliseconds: 100), 1.0, const Duration(seconds: 5), null, null, printError: true), null);
    });
  });
  group('Other Test', () {
    test('=> crc32', () {
